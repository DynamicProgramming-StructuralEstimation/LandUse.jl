...
function jm()
	...
	# τ(x,θu,a,taum,taul) = a * θu^(taum) * x^(taul)
	# wu(L,Lr,cons,eta) = cons * (L - Lr)^eta


	# register those functions
	# register(m, :τ, 5, τ, autodiff=true)
	# register(m, :wu, 4, wu, autodiff=true)
	...
	@NLexpression(m, r_pr_cbar, r - pr * p.cbar + p.sbar )
	@NLexpression(m, xsr, wr + r_pr_cbar )

	@NLexpression(m, nodes[i = 1:p.int_nodes], ϕ / 2 + ϕ / 2 * mo.inodes[i] )
	@NLexpression(m, τ[i = 1:p.int_nodes], p.a * p.θu^(p.taum) * nodes[i]^(p.taul) )
	@NLexpression(m, q[i = 1:p.int_nodes], qr * ((wu0 - τ[i]) / xsr)^(1.0/p.γ))
	@NLexpression(m, H[i = 1:p.int_nodes], q[i]^p.ϵr)
	@NLexpression(m, h[i = 1:p.int_nodes], p.γ * ((wu0 - τ[i]) + r_pr_cbar) / q[i])

	@NLexpression(m, iDensity, (ϕ/2) * sum(mo.iweights[i] * H[i] / h[i] for i in 1:p.int_nodes))


	# @NLexpression(m, Srh, p.γ / (1 + p.ϵr) * (wu + (ρ * (p.S - p.λ) / p.L) - pr * p.cbar + p.sbar) * Lr / ρ)
