...
function popdata(; maxrank = 20)
    ...
    d1[d1.relpop .<  0.13, :group] .= 3
    d1[d1.relpop .<  0.043, :group] .= 4
    d1[d1.relpop .<  0.029, :group] .= 5
    ...
    pl2 = @df subset(d20, :group => x -> x .> 1) plot(:year, :relpop, group = :LIBGEO, marker = :circle, title = "Population Data relative to Paris")
