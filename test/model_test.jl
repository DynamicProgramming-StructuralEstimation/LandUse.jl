@testset "Single Region" begin
	...
	@testset "components in general model checks" begin
		...
		p = LandUse.Param()
		r0 = nlsolve_starts(LandUse.startval(),p=p)
	...
	@testset "test flat elasticity ϵs = 0" begin
		...
		r = LandUse.nlsolve((F,x) -> LandUse.solve!(F,x,p,m),r0.zero,iterations = 100)
		rf = LandUse.nlsolve((F,x) -> LandUse.solve!(F,x,p,fm),r0.zero,iterations = 100)
