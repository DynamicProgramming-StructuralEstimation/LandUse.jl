@testset "Single Region" begin
	@testset "components in flat model checks" begin
		...
		@test LandUse.cu(0.0,p,m) == (1.0 - p.γ)*(1.0 - p.ν)*(LandUse.w(m.Lu,0.0,m.ϕ,p) .+ (m.r + p.sbar - m.pr * p.cbar))
		@test LandUse.cu(0.9,p,m) == (1.0 - p.γ)*(1.0 - p.ν)*(LandUse.w(m.Lu,0.9,m.ϕ,p) .+ (m.r + p.sbar - m.pr * p.cbar))
	...
	@testset "test flat elasticity ϵs = 0" begin
		...
		@testset "equation (26) special case" begin
			...
			@test (1 - p.ν) * (1 - p.γ) * (yy  - fm.pr * p.cbar) + p.ϵr * fm.r ≈ (1 - p.ν) * (1 - p.γ) * (LandUse.pcy(m,p) + p.sbar - m.pr * p.cbar) + p.ϵr * m.r atol=0.02
		end
	...
	# 			# equation (25)
	# 			@testset "Walras' Law" begin
	# 				@test p.ν * (1 - p.γ) * (LandUse.pcy(M[it],p) + p.sbar) + M[it].pr * p.cbar * (1.0 - p.ν * (1 - p.γ)) ≈  M[it].pr * LandUse.Yr(M[it],p) / p.L
