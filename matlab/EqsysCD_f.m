function F=EqsysCD_f(x)
% Equilibrium equations when Cobb-Douglas preferences, linear commuting costs tau*d
...
Shr=gama2/(q1*chi1)*(w1+r-p1*cbar+sbar)*L1; %rural land used for residential purposes (housing plus wasted when chi1<1).
...
F(3)=1+tau*w2*L2/(chi2*q1)-((w2+r-p1*cbar+sbar)/(w2*(1-tau*phi)+r-p1*cbar+sbar))^(1/gama2); %urban workers accomodation (mtb: isn't q1 multipled or exponentiated to sth??)
...
%F(6)=L1*(nu*(1-gama)*(w1+r-p1*cbar))...
%    +nu*(1-gama)*chi2*q1/((1+gama)*tau*w2)*((w2+r-p1*cbar)^(1/gama+1)/((w2*(1-tau*phi)+r-p1*cbar)^(1/gama))-(w2*(1-tau*phi)+r-p1*cbar))...
%    +p1*cbar*L...
%   -p1*theta1*L1^alpha*S1^(1-alpha); %farm good market clearing condition
 
% F(6)=L1*(nu*(1-gama)*(w1+r-p1*cbar)+cbar)...
%     +nu*(1-gama)*chi2*q1/((1+gama)*tau*w2)*((w2+r-p1*cbar)^(1/gama+1)/((w2*(1-tau*phi)+r-p1*cbar)^(1/gama))-(w2*(1-tau*phi)+r-p1*cbar))...
%     +chi2*q1*p1*cbar/(tau*w2)*(((w2+r-p1*cbar)/(w2*(1-tau*phi)+r-p1*cbar))^(1/gama)-1);
%     -p1*theta1*L1^alpha*S1^(1-alpha); %farm good market clearing condition
