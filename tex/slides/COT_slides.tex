...
\date{Urban Economics Association [Virtual] October 2020}
...
\begin{column}{0.7\textwidth}
	...
	\includegraphics[height=0.85\textheight]{\dataplots/Toulouse-sat-area.png}\end{adjustbox}
	\end{column}
\begin{column}{0.3\textwidth}
\begin{midi}
...
\includegraphics[width=0.9\textwidth]{\dataplots/densities-time-log-wtd.pdf}
\end{column}
...
\medskip
\noindent \textbf{Commuting costs}. $\eta_\ell=\frac{1}{3}, \eta_m=\frac{2}{3}$, $\zeta = 0.5$.

\medskip
\noindent \textbf{Housing supply elasticity.} Baseline with $\epsilon_{\ell} = \epsilon_r=4$. Version with lower $\epsilon_{\ell}$ at CBD.

\medskip
\noindent \textbf{Population}. Actual population growth data extrapolated to 2050 using central INSEE scenario.
\end{frame}

\begin{frame}{Baseline Model: Empirical Targets}

\begin{columns}
\begin{column}{0.3\textwidth}
\begin{widee}
\item Top row: Structural Change moments
\item Bottom row: City structure moments
\item Work in Progress (missing \emph{data} lines...)
\end{widee}
\end{column}
\begin{column}{0.7\textwidth}
\includegraphics[width=1\textwidth]{\modelplots/issue36-baseline-fit.pdf}
\end{column}\end{columns}

\end{frame}

\begin{frame}[label=res_laborland]
\frametitle{More Results in Baseline Model}

\begin{columns}
\begin{column}{0.3\textwidth}
\begin{mide}
\item Rural population: 78\% $\rightarrow$ 2\%
\item Rural spending share: 77\% $\rightarrow$ 1.8\%
\item Urban Density: Falls by factor of 8.
\item Evolution of Land Rent Shares $\rightarrow$ Piketty.
\end{mide}
\end{column}
\begin{column}{0.7\textwidth}
\begin{adjustbox}{center}
...
\begin{frame}{Counterfactual 1: High Urban Productivity Growth Only}

\begin{columns}
\begin{column}{0.3\textwidth}
Rural productivity is very low. Hence:
\begin{mide}
\item Need too many rural workers.
\item High price of food. 
\item High value of farmland. 
\item City people live in small houses.
\end{mide}
\end{column}
\begin{column}{0.7\textwidth}
\begin{adjustbox}{center}
...
\begin{columns}
\begin{column}{0.3\textwidth}
Meanwhile, Urban growth is high:
\begin{mide}
\item The opportunity cost of commute is rising ($w_u$ is rising).
\item Average density is falling.
\item But relatively more in center than at fringe!
\end{mide}
\end{column}
\begin{column}{0.7\textwidth}
\begin{adjustbox}{center}
\includegraphics[width=\textwidth]{\modelplots/issue36-u-fast2.pdf}\end{adjustbox}
\end{column}\end{columns}
\end{frame}

\begin{frame}{Counterfactual 2: High Rural Productivity Growth Only}

\begin{columns}
\begin{column}{0.3\textwidth}
\begin{midi}
\item Rural \emph{push} gets labor allocation right.
\item Relative price dynamics qualitatively correct.
\item Spend quite a lot on housing though. What's going on?
\end{midi}
\end{column}
\begin{column}{0.7\textwidth}
\begin{adjustbox}{center}
...
\begin{column}{0.3\textwidth}
\begin{midi}
\item No urban growth $\Rightarrow$ opportunity cost of commute constant.
\item People have no strong incentive for faster commute.
\item Rural land value $\downarrow$ $\Rightarrow$ fringe density $\downarrow$...
\item ... but $\tau(\ell) / w_u \approx$ constant $\Rightarrow$ central density $\uparrow$...
\end{midi}
\end{column}
\begin{column}{0.7\textwidth}
\begin{adjustbox}{center}
\includegraphics[width=1\textwidth]{\modelplots/issue36-r-fast2.pdf}\end{adjustbox}
\end{column}\end{columns}
\end{frame}
