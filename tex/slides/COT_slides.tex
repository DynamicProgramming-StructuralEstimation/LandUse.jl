...
\framesubtitle{Real Housing Price Index in France (1870-2010): Knoll et al. (2017)}
	\begin{center}
		\includegraphics[scale=0.65]{\imgs/hpreal100_FRA.pdf}
	\end{center}
\end{frame}
...
\re{$\underline{c},\underline{s}$ subsistence consumption level} for the rural and urban goods, respectively.\\
...
\item<3-> $w_u(1-\tau(\ell))$ urban wage
...
\begin{frame}{Commuting Costs}

\begin{columns}
\begin{column}{0.5\textwidth}
\begin{midi}
\item $\tau(\ell)$ is composed of a fixed operating cost $o$ and time cost $t(\ell)$
\begin{equation*}
\tau(\ell) = o + \zeta w_u t(\ell)
\end{equation*}
\item Commuting time with transport mode $m$ (speed): $\frac{\ell}{m}$
\item Operating cost $o(\ell,m)$ is s.t. longer ($\uparrow \ell$) and faster ($\uparrow m$) commutes are more expensive.
\item Train line vs horse-drawn carriage.
\end{midi}
\end{column}
\pause
\begin{column}{0.5\textwidth}
\begin{midi}
\item Assume $o(\ell,m) = \frac{c_\tau}{\eta_m} m^{\eta_m} \ell^{\eta_\ell}$ 
...
{\tiny Source: Carte Etat-Major, IGN and GHSL. Census. Central Paris = Districts 1 to 6.}
\end{frame}
